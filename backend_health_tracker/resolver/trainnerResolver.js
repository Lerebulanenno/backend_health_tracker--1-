...
export const trainnerResolver = {
  Query: {
    ...
    trainner: (_, args) =>
      new Promise((resolve, reject) => {
        db.query(
          "SELECT * FROM tb_trainner WHERE id_trainner = ?",
          [args.id_trainner],
          (err, results) => {
            if (err) reject(err);
  ...
  Mutation: {
    ...
    updateTrainner: async (_, args) => {
      try {
        ...
        await new Promise((resolve, reject) => {
          db.query(
            `UPDATE tb_trainner SET username = ?, gmail = ?, password = ? WHERE id_trainner = ?`,
            [args.username, args.gmail, hashedPassword, args.id_trainner],
            (err, result) => {
              if (err) reject(err);
              else resolve(result);
            }
